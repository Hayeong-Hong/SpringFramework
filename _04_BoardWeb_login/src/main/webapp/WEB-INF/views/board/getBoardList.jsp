				...
				<tr>
					<td align="right">
						<select name="searchCondition">
							...
							>전체</option>
							<option value="title"
							<c:if test="${searchCondition eq 'title'}">selected</c:if>
							>제목</option>
							<option value="content"
							<c:if test="${searchCondition eq 'content'}">selected</c:if>
							>내용</option>
							<option value="writer"
							<c:if test="${searchCondition eq 'writer'}">selected</c:if>
							>작성자</option>
						</select>
		...
		<table border="1" style="width: 700px; border-collapse: collapse;">
			...
			<c:forEach var="board" items="${boardList }">
				<tr>
					...
					<td><a href="/board/getBoard.do?boardSeq=${board.boardSeq }">${board.boardTitle }</a>
					</td>
